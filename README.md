...
2. **Set MongoDB URI** (never commit credentials):

   cp .env.example .env
   # Edit .env and add: MONGO_URI=mongodb+srv://user:password@cluster.mongodb.net/...

3. **Run locally**: `python app.py` (or `nohup python3 app.py > log.txt &` on a server)
